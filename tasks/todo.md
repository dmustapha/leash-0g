...
- [ ] Backend: `getErc20Meta` chain read; store fee token symbol+decimals on the job row at originate (migration 011, untracked); `formatAsset(baseUnits,decimals,symbol)`; redesign verdictGate approval card copy (release-gate framing + job question + correct asset) + SSE + §8 digest amounts.
- [ ] FE: settlement/approval card component renders the true asset; redesign layout/copy.
